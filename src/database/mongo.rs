...
impl DatabaseClient {
    async fn new(conf: &DbConfig) -> Result<Self>{
        ...
        let client = mongodb::Client::with_uri_str(
            &conf.connection_string).await?;

        let withdraw_items = Box::new(client.database(&conf.database)
            .collection("userBridgeWithdrawableBalanceItem"));
        let validator_signatures = Box::new(client.database(&conf.database)
    ...
    async fn add_signature_to_withdraw_item(
        ...
        let res: Option<Document> = self.withdraw_items.find_one_and_update(
            doc! {
                "$and": [
                    { "network": network, },
    ...
    async fn signed_swaps(
        ...
        let mut cursor: Cursor<_> = self.validator_signatures.find(
            doc! {
                "$and": [
                    { "network": network, },
                    { "transactionId": transaction_id },
